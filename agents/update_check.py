...
class UpdateCheckAgent(BaseAgent):
    ...
    async def _check_now(self) -> None:
        try:
            ...
            if latest and self._is_newer(latest, local):
    ...
    def _is_newer(self, latest: str, local: str) -> bool:
        """Return True if *latest* represents a newer semantic version."""
        try:
            def parse(ver: str) -> list[int]:
                return [int(p) for p in ver.lstrip("v").split(".")]

            return parse(latest) > parse(local)
        except Exception:
            return latest != local
